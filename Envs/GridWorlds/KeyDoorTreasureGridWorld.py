...
class KeyDoorTreasureGridWorld(gym.Env):
    ...
    def gt(self, max_dist_accuracy=None):
        ...
        if max_dist_accuracy is not None:
            d_gt_same = torch.clamp(d_gt_same, max=max_dist_accuracy)
            d_gt_same_reverse = torch.clamp(d_gt_same_reverse, max=max_dist_accuracy)
            d_gt_wo_obj = torch.clamp(d_gt_wo_obj, max=max_dist_accuracy)
            d_gt_w_obj = torch.clamp(d_gt_w_obj, max=max_dist_accuracy)
